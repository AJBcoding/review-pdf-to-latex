# Unified Comment Model & Multi-Format Round-Trip — Specification Draft

| Field | Value |
|---|---|
| Title | Unified cross-format comment/annotation model and native round-trip (PDF / DOCX / MD / HTML) |
| Date | 2026-06-12 |
| Status | DRAFT — Phase B lane B2 output. GATE 1 OPEN (approved by Anthony Byrnes 2026-06-12); GATE 2 CLOSED — **no implementation may begin from this document**. The Phase-B closer promotes this draft to `docs/specs/2026-06-12-unified-comment-model-and-roundtrip.md`. |
| Author | Claude (Fable 5), Phase B lane B2, on behalf of Anthony Byrnes |
| Repo | `/Users/anthonybyrnes/PycharmProjects/review-pdf-to-latex/` |
| Primary input | `docs/reports/arch-review/2026-06-12/REVIEW.md` (Phase A, ~57 findings, 14/14 verification) — esp. the Capability section and its "DRAFT — input to Phase B" sketch |
| Raw evidence | `evidence/pass5/5B.md` (assets + sketch), `evidence/pass5/5A.md` (options + trade-offs) |
| Related — design | [`docs/specs/2026-05-16-review-pdf-to-latex-design.md`](../../../../specs/2026-05-16-review-pdf-to-latex-design.md) (engine contract §5–§9 authoritative), [`docs/specs/2026-05-19-electron-app-ux-spec.md`](../../../../specs/2026-05-19-electron-app-ux-spec.md) |
...
The centerpiece is the **M-2 per-comment discriminated anchor union** (REVIEW.md Capability §3; options analysis in 5A §iii), replacing today's required PDF-shaped anchor on every comment in every format (types.ts:130, verified C5/C6). It is a **three-party schema event** — drafts sidecar, submit/results files, and the rig contract move together, because `ResultEntry.new_anchor` is PDF-typed today (types.ts:245, verified C12). The per-format adapters are second instantiations of two proven in-repo templates: the bundle.ts six-step pipeline (5B A3) and the pdfHealth wrap-a-subcommand precedent (5B A8). The hardest non-code problem — three status vocabularies sharing zero strings (Capability §4) — is resolved here by a single mapping table whose previously-open `?` cells get PROPOSED DEFAULTS.
...
**Out of scope (other Phase-B lanes or later):** the renderer decomposition that hosts this model (`openDocument`/`DocSession`/format registry — REVIEW.md "Renderer — Orchestration & Chrome"); the five shipping data-loss bugs C1/C2/C7/C8/C9 (sequenced by the roadmap, not this spec — though §4.4 names the two that are hard prerequisites); the Claude-route convergence; the legacy HTTP viewer keep-vs-kill (touched only where it intersects §7); any visual in-place PDF annotation-editing UX (pdf.js AnnotationEditorLayer deliberately not evaluated — REVIEW.md Capability "Also noted"; 5A out-of-scope notes).

**Evidence basis:** REVIEW.md findings are cited by section/claim id (C1–C14 were independently re-verified in Pass 6). Schema lines quoted in this spec were re-opened in this session: `desktop/shared/types.ts:110-192` (CommentStatus, AnchorRegion, CommentPayload, AnchorKind, DocFingerprint, DraftsFile) and `:211-270` (SubmitFile, ResultEntryStatus, ResultEntry, ResultsFile); `src/review_pdf_to_latex/state.py:173-183` (engine `Status` Literal, 9 values).
...
// REVIEW.md "shared/types.ts is a five-concern god-file")
...
3. **`text-quote` is ONE kind shared by MD, HTML, and DOCX body text.** Same resolver (`fuzzyMatchAnchor`, shared/md/anchors.ts — pure, tested, with the Pass-4B verification guard added as a prerequisite), three formats (5B A5).
...
The v2 record is today's `CommentPayload` (types.ts:126-166, re-opened this session) restructured per the 5B Part-3 sketch, plus the three round-trip-driven blocks 5A added (`origin`, `native`, union-typed lineage):
...
interface CommentV2 {
  ...
  native?: {
    ...
    in_reply_to?: string;                // reply parent — captured nowhere today (Pass-2 reusability note)
...
- **`native` is the normative native block**, field set adopted from the spike's readback records `{page, subtype, rect, contents, author, color, quads[]}` (readback.json:1-42; 5B F4) — the only complete native-annotation record in the repo today.
...
**Migration (v1 → v2), in main, at read time** — following the two established in-repo precedents: the tolerant reader + one-shot rewrite of sidecar-migration.ts (domain trap #5; Pass 3C), and the documented default "existing sidecars without `anchor_kind` default to `'pdf-glyph-rect'`" (types.ts:168-169 comment, re-opened):

1. `drafts:read` detects `schema_version: 1` and migrates rows **in memory**; the file is rewritten as v2 only on the next `drafts:write` (no startup-blocking sweep — the standing sidecar-migration startup-cost finding, Pass 3C tail, is the anti-pattern to avoid repeating).
2. Per-row mapping, exhaustive over the four real v1 shapes Pass 4B identified:
...
3. The migrated write must be **atomic and clobber-safe**: it goes through the single extracted `atomicWriteJson` (the 5×-duplication finding, REVIEW.md "Persistence & Submit") and must not interact with the C8 re-seed path — the seeding-idempotence fix (read-disk-before-seed) is a hard prerequisite (§4.4).
4. Vitest coverage for all four row shapes plus an already-v2 passthrough, in the style of sidecar-migration.test.ts (the best-covered desktop module — REVIEW.md Pass-3 takeaway).
...
Both files already carry optional `schema_version?: 1` (types.ts:212, :258 — re-opened), so versioning is additive (5B F3).
...
**Old-payload tolerance, both directions, forever:** a bare `{ page, region }` object (no `kind`) appearing in any `new_anchor` is structurally unambiguous and is read as `{ kind: 'pdf-quad', ... }`. This single tolerant-parse rule means v1 results files on disk (and a not-yet-updated rig emitting v1-shaped relocations for PDF rounds) keep working without a flag day.
...
|---|---|---|---|
| 0 | desktop | **Prerequisite fixes**: C8 seed-idempotence (the migration rewrite must not ride a known clobber path); C5 narrowing — `writeBundle` rejects/skips non-`pdf-quad` anchors *immediately*, even before v2, as a one-line guard | — |
| 1 | desktop | DraftsFile v2 + tolerant v1 reader + migration (§3.3); `shared/comments.ts` split out of types.ts; all renderer payload builders collapse to one `buildPayload(buf, anchor)` (REVIEW.md "Comments, Submit UX" rec) | v1 sidecars (reads + migrates); still EMITS v1 SubmitFile |
...
| 3 | desktop | flips SubmitFile writer to v2 (after step-2 rig is confirmed deployed) | v1 AND v2 results files, permanently (the §4.3 tolerant-parse rule) |
...
PROPOSED DEFAULT — owner may override at GATE 2: no runtime version *negotiation* (e.g. rig advertising capabilities). With a single owner operating both sides, deploy-order discipline per the table is cheaper than a handshake protocol; revisit if a second rig operator ever exists.
...
interface FormatCommentAdapter {
  ...
  capabilities: {
    ...
    replies: boolean;        // thread support (PDF /IRT pending spike S-1; DOCX false in v1)
...
Prerequisites that ride this spec (both from the verified Pass-4B fuzzy-re-anchoring finding):
...
The three vocabularies, re-verified this session: engine `Status`, 9 values (state.py:173-183); desktop `CommentStatus`, 7 values (types.ts:110-117); rig `ResultEntryStatus`, 5 values, a declared subset of CommentStatus (types.ts:227-235 comment "narrower than CommentStatus"). Grep-proven zero shared strings with the engine (Capability §4), with near-miss names (`needs_review` vs `needs-followup`).
...
**The normative mapping table.** The two `?` cells from the GATE 1 packet get PROPOSED DEFAULTS, marked:
...
| — (not a unified status) | `needs_review` | — | **PROPOSED DEFAULT — owner may override at GATE 2.** Engine `needs_review` is mapping-confidence/apply-failure workflow ("low-confidence mappings" bucket, design spec §3.2 table; "apply-failure flow" per Capability finding 4). In the unified model it surfaces as `workflow.engine_status: 'needs_review'` plus the existing orphan/confidence machinery on the anchor (`fuzzyMatchAnchor`'s `'orphaned'` confidence) — a property of the *anchor resolution*, not a disposition. |
...
**Interaction with keep-vs-kill (owner decision, still open per the GATE 1 packet):** nothing in this spec depends on the legacy HTTP viewer surviving. The one engine asset this spec's direction wants regardless is the flock-disciplined event bus extraction (`events.py`, REVIEW.md "Keep-vs-kill: split the format-agnostic event bus") — noted as a compatible-but-independent workstream, not a dependency.

## 8. Open spikes and kill criteria

Each spike is time-boxed at 1 day, has a binary kill criterion, and ships its result as a dated research note (house pattern: `docs/research/2026-05-21-pdf-lib-annotation-spike/`). All three were flagged feasible-unproven in the GATE 1 packet UNCERTAIN list; none may be skipped by assuming success.

**S-1 — PDF /IRT reply chains (write side).**
Scope: with pdf-lib, write a Text annotation carrying /IRT pointing at an existing Highlight; round-trip the file through Acrobat (or Preview + pdf.js `getAnnotations`) and confirm the chain displays as a reply and survives a re-save. The 2026-05-21 spike did NOT exercise /IRT (5A (i) flagged risk (a); PDF 1.7 §12.5.2 territory).
Kill criterion: if a written chain fails to display as a reply in Acrobat, or pdf-lib cannot reliably address /IRT targets, within the day → **v1 ships replies read-only** (display `native.in_reply_to` threads in the card stream; no reply write-back), `capabilities.replies = false` on the PDF adapter, and a bead records the retry condition (next pdf-lib release or a hand-built dict workaround).

**S-2 — per-line quads write-back.**
Scope: upgrade the writer from single-bbox /QuadPoints to per-line quads using the capture side's existing `screenRects` (pdf-viewer.ts:60; merge logic :447-463 — 5A (i) flagged risk (b)); verify Acrobat renders multi-quad highlights correctly, including on a restored/degraded PDF (domain trap #8 class).
...
|---|---|---|---|
| D1 | Per-comment discriminated anchor union (M-2) with `quads[]`, `origin`, `native_ref`; M-1 accretion rejected | Settled by verified findings (C5, C6, C12; 5A §iii) — owner accepted M-2 as Phase-B centerpiece at GATE 1 (packet checklist item) | REVIEW.md Cap §3 |
| D2 | Three-party rollout order: sidecar v2 → rig reader/writer → desktop submit-writer flip; bare `{page,region}` parsed as pdf-quad forever | Settled mechanics (forced by file-reader topology); the no-handshake choice is **PROPOSED DEFAULT — owner may override at GATE 2** | §4.4; C12; 5B F3 |
...
| D5 | `needs_review` ≠ `needs-followup`: engine `needs_review` is anchor-resolution workflow, never a unified disposition | **PROPOSED DEFAULT — owner may override at GATE 2** (closes a GATE-1 `?` cell) | §6 table; Cap §4 |
...
| D10 | Replies (PDF /IRT), multi-quad writes, and DOCX marker insertion ship only behind spikes S-1/S-2/S-3 with the stated kill paths | Settled (feasibility was flagged unproven in the GATE 1 packet; spiking is the mitigation) | §8; 5A (i)/(ii) |
...
1. A v1 sidecar of each of the four real anchor shapes (pdf, md, smuggled-html, pdf_annotation_id-bearing) opens, migrates in memory, and round-trips to a valid v2 file; vitest covers all four + v2 passthrough (§3.3).
...
5. v1 results files and bare-`{page,region}` `new_anchor`s remain readable at every rollout step; the results watcher is diff-identical (§4).
6. Spikes S-1/S-2/S-3 each have a dated research note with a PASS or KILL outcome before their gated features are filed as beads (§8).
