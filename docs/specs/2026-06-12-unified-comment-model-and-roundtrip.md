...
| Decision marking | Every unsettled point is marked **PROPOSED DEFAULT — owner may override at GATE 2**. Points the owner has since signed off are marked **OWNER-CONFIRMED (OD-1, …)** — currently the entire §6 status-vocabulary table and decision-log rows D4/D5/D6/D11 (owner sign-off 2026-06-14, `bd show rev-od1`). Unmarked statements rest on verified Phase-A findings. |
...
- **`status` stays the 7-value desktop enum.** OWNER-CONFIRMED (OD-1, D4, 2026-06-14; see §6): the unified vocabulary IS `CommentStatus` (types.ts:110-117) extended by nothing; engine statuses are *mapped* at the boundary (§6, §7) rather than merged into one super-enum. Rationale in §6.
...
> **OWNER-SIGNED — this table is final (OD-1, rev-od1).** On 2026-06-14 the owner (Anthony Byrnes, via `review_pdf_to_latex/crew/anthony`) confirmed every status-vocabulary default in this section **as-is, with no overrides** — decision-log rows **D4, D5, D6, D11** (`bd show rev-od1`). The cells that were `?` in the GATE 1 packet are now settled; the `PROPOSED DEFAULT — owner may override at GATE 2` markings within §6 are replaced below by `OWNER-CONFIRMED (OD-1)`. The mapping is **complete**: every one of the engine's 9 `Status` values (state.py:173-183, re-verified per L13 status-enum export) has a destination in the table. This sign-off settles the *vocabulary*; it does **not** open GATE 2 — no adapter or mapping code is written against this table until GATE 2 opens (see §4 acceptance criterion 4).
...
**Architecture of the resolution (OWNER-CONFIRMED — OD-1, D4, 2026-06-14):** the unified vocabulary is the **desktop `CommentStatus`, unchanged** — no merged super-enum. Engine statuses are a different *kind* of thing (per-annotation workflow state inside the apply/build loop) and are **mapped at the engine↔desktop seam** (§7), with the engine-native status preserved losslessly in an optional `workflow.engine_status` passthrough field so every fold below is reversible. Rationale: merging would force the 9-value engine workflow grammar into every drafts sidecar that will never touch the engine (today: all of them — the two worlds share a directory but not one file, 5B F1), and the near-miss collision (`needs_review`/`needs-followup`) is *eliminated* rather than adjudicated when the engine string never enters the desktop enum.

**The normative mapping table.** The `?` cells from the GATE 1 packet are now OWNER-CONFIRMED (OD-1, 2026-06-14), marked per cell below. (This table is the single source — the contradictory draft default that mapped `needs_review` toward a "`build-failed` family" was withdrawn at challenge; see B4 dispositions 1–2.)
...
| D4 | Unified status vocabulary = desktop `CommentStatus` unchanged; engine statuses mapped at the seam, preserved in `workflow.engine_status` | **OWNER-CONFIRMED (OD-1, rev-od1, 2026-06-14)** — accepted as-is, no override | §6; 5B F1 |
| D5 | `needs_review` ≠ `needs-followup` ≠ `build_failed`: engine `needs_review` is anchor-resolution workflow, never a unified disposition and never the rig's build outcome | **OWNER-CONFIRMED (OD-1, 2026-06-14)** — closes a GATE-1 `?` cell; supersedes the withdrawn draft default per B4 disposition 1 | §6 table; Cap §4; types.ts:247-249 |
| D6 | `accepted`/`redrafted` fold into unified `applied` (lossless via `workflow.engine_status`); `surfaced_pending` maps to `open` + `engagement_level:'surface'`; no unified `resolved` value | **OWNER-CONFIRMED (OD-1, 2026-06-14)** — closes the remaining GATE-1 `?` cells | §6 table; types.ts:105; 5B §3.3 |
...
| D11 | Keep shipped status spellings verbatim (incl. `build_failed`); keep `bundle_pdf/bundle_json` as deprecated aliases through the transition | **OWNER-CONFIRMED (OD-1, rev-od1, 2026-06-14)** — accepted as-is, no override | §6 lexical note; §4.2 |
